...
def categorias():
    ...
    # verifica se o usuário está logado para buscar seus itens
    # senão tras todos os itens
...
def itens_categoria(categoria_id):
    ...
    if current_user.is_authenticated:
        ...
        total_itens = db.session.query(ItemCategoria) \
            .filter_by(categoria_id=categoria_id).count()
...
def editar_item_categoria(categoria_id, item_id):
    ...
    categoria = db.session.query(Categoria).filter_by(id=categoria_id).one()
    item_editado = db.session.query(ItemCategoria) \
        .filter_by(categoria_id=categoria.id).filter_by(id=item_id).one()
    ...
    return render_template("item_categoria/editar-item.html",
                           categoria_id=categoria_id,
                           item_id=item_id, item=item_editado,
                           categorias=categorias)
